...
waveforms.sinusoidal.frequency=35.*10^9;
% waveforms.cosine_modulated_gaussian(1).bandwith=1e9;%0.55
% waveforms.cosine_modulated_gaussian(1).modulation_frequency=waveforms.sinusoidal.frequency;%2.75e9;
...
%% FDTD variables
number_of_cells_per_wavelength=20;%180
...
boundary.air_buffer_number_of_cells_zn=ceil(0.02./dz)+20;%20+40+40+40;
boundary.cpml_number_of_cells_zn=20;

boundary.type_zp='cpml';
boundary.air_buffer_number_of_cells_zp=ceil(0.10./dz);%20+40+40+40;%+40+200;%+200+200+200+200+200+200+200+200+200;%+200+200+200+200+200+200+200+200
...
material_type(2).sigma_e=1e20;
material_type(2).sigma_m=1;
...
%% update source
Ex(1:nx,1:ny,n_cpml_zn+10)=Ex(1:nx,1:ny,n_cpml_zn+10)...
    -sin(omega.*number.*dt);%+cos(2*pi*frequency*(number.*dt-t_0)).*exp(-((number.*dt-t_0)/tau).^2);%-sin(omega.*number.*dt);% Differentiated Gaussian pulse
...
%     E_1./E_0P
%     end
if mod(number,50)==0||number==number_of_time_steps
% 
